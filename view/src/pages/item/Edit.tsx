...
import { fetchItem, updateItem } from './slices';
...
const Edit = () => {
    ...
    useEffect(() => {
        setFormData({
            ...
            proration_type_id: item?.data?.proration_type?.id && String(item?.data?.proration_type?.id),
        })
    }, [dispatch, item])
            ...
            <div className="w-md max-w-md items-center border rounded-2xl p-6">
                ...
                <form onSubmit={handleSubmit} className='space-y-4'>
                    <div className='grid grid-cols-2 gap-2 items-center' >
                        ...
                        <Switch disabled={item.loading} name='active' onCheckedChange={(checked) => setFormData({ ...formData, active: checked as boolean })} checked={formData.active} id='active' />
                    </div>
                    <div className='grid grid-cols-2 gap-2 items-center'>
                        <Label htmlFor='proration_type_id' >Proration type</Label>
                        <Select disabled={item.loading} name='proration_type_id' onValueChange={(value) => setFormData({ ...formData, proration_type_id: value })} value={formData.proration_type_id}>
                            <SelectTrigger>
                    ...
                    <div className="flex gap-4">
                        ...
                        <Button size='sm' variant='outline' className='mt-2' onClick={() => navigate('/item/list')}>Cancel</Button>
                    </div>
