...
import { fetchOrder, updateOrder } from './slices';
...
const Edit = () => {
    ...
    useEffect(() => {
        setFormData({
            active: order?.data?.active || false,
            proration_type_id: order?.data?.proration_type?.id && String(order?.data?.proration_type?.id),
        })
    }, [dispatch, order])
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await dispatch(updateOrder({ id, formData })).unwrap();
            navigate('/order/list')
        } catch (error) {
            toast.error(error);
        }
            ...
            <div className="w-md max-w-md items-center border rounded-2xl p-6">
                ...
                <form onSubmit={handleSubmit} className='space-y-4'>
                    <div className='grid grid-cols-2 gap-2 items-center' >
                        ...
                        <Switch disabled={order.loading} name='active' onCheckedChange={(checked) => setFormData({ ...formData, active: checked as boolean })} checked={formData.active} id='active' />
                    </div>
                    <div className='grid grid-cols-2 gap-2 items-center'>
                        <Label htmlFor='proration_type_id' >Proration type</Label>
                        <Select disabled={order.loading} name='proration_type_id' onValueChange={(value) => setFormData({ ...formData, proration_type_id: value })} value={formData.proration_type_id}>
                            <SelectTrigger>
                    ...
                    <div className="flex gap-4">
                        ...
                        <Button size='sm' variant='outline' className='mt-2' onClick={() => navigate('/order/list')}>Cancel</Button>
                    </div>
